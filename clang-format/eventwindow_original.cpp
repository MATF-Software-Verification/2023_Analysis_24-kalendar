    ...
    ui(new Ui::EventWindow)
{
    ...
    if (m_attachmentManager->auth()->loggedIn("tokenDrive.json")){
        m_attachmentManager->auth()->refreshAccessTokenAtBeginning("tokenDrive.json");
    ...
    connect(m_attachmentManager, &AttachmentManager::fileUploaded, this, &EventWindow::onFileUploaded);

}

void EventWindow::setEvent(QString title, QDate startDate, QDate endDate, QTime startTime, QTime endTime, QString description,
...
void EventWindow::on_pbSave_clicked()
{
    ...
    if((ui->deStart->date() > ui->deEnd->date()) ||
            (ui->deStart->date() == ui->deEnd->date() && ui->teStart->time() > ui->teEnd->time())){
        ErrorMsg msgE(this);
...
void EventWindow::on_pbBrowseAttachment_clicked() {
    ...
    QFile* file = new QFile(QFileDialog::getOpenFileName(this, "Choose attachment", QStandardPaths::writableLocation(QStandardPaths::DesktopLocation)));
...
void EventWindow::on_pbLogin_clicked()
{
    ...
    connect(m_attachmentManager, &AttachmentManager::accessGrantedSignal, this, &EventWindow::showAttachmentWidgets);
}
