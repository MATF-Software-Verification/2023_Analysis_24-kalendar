...
CalendarManager::CalendarManager(QObject *parent, OAuth2* oauth2) : QObject(parent), m_auth(oauth2)
{
    auto replyHandler = new QOAuthHttpServerReplyHandler(m_auth);
    m_auth->getGoogle()->setReplyHandler(replyHandler);
    connect(this, &CalendarManager::savedCalendarIdsToJsonObject, &CalendarManager::onSavedCalendarIdsToJsonObject);
    connect(this, &CalendarManager::savedCalendarEventsToJsonObject, &CalendarManager::onSavedCalendarEventsToJsonObject);
    connect(m_auth->getGoogle(), &QOAuth2AuthorizationCodeFlow::tokenChanged, this, &CalendarManager::onTokenChanged);
...
void CalendarManager::onSavedCalendarEventsToJsonObject() {
    ...
    for (auto item : items) {
        ...
        Event* event = new Event(id, title, description, startDate, endDate, startTime, endTime);
        ...
        for (auto attachment : attachments) {
            AttachmentInfo* at = new AttachmentInfo(attachment.toObject()["fileId"].toString(), attachment.toObject()["fileUrl"].toString(),attachment.toObject()["mimeType"].toString(), attachment.toObject()["title"].toString(), attachment.toObject()["iconLink"].toString());
...
OAuth2 *CalendarManager::getAuth() const
{
    return m_auth;
}
...
const QString CalendarManager::fromEventToString(Event *event)
{
    ...
    if(event->lastsAllDay) {
        ...
        QDateTime startDT(event->get_startDate(), event->get_startTime()), endDT(event->get_endDate(), event->get_endTime());
    ...
    if(event->getUseDefaultAlert())
        str += QString("\"reminders\": {\n\"useDefault\": \"%1\"},\n").arg("true");
         ...
         if(!event->getNoAlert())
            str += QString(",\n \"overrides\": [\n{\n \"method\": \"%1\",\n\"minutes\": \"%2\" \n}\n]").arg(event->getAlertType()).arg(event->getAlertNum());
...
void CalendarManager::createEvent(Event* event) {
    ...
    auto reply = m_auth->getGoogle()->post(QUrl(QString("https://www.googleapis.com/calendar/v3/calendars/%1/events%2").arg(m_calendarId).arg(at)), params);
    connect(reply, &QNetworkReply::finished, [=](){
        qDebug() << "POST REQUEST FINISHED. Error? " << (reply->error() != QNetworkReply::NoError);
...
void CalendarManager::updateEvent(Event* event) {
    ...
    //QByteArray params = fromEventToString(event).toUtf8();
    /*qDebug() << params;
    auto reply = m_auth->getGoogle()->put(getUrlEvent(m_calendarId, event->getId()), params);
    connect(reply, &QNetworkReply::finished, [=](){
        qDebug() << "PUT REQUEST FINISHED. Error? " << (reply->error() != QNetworkReply::NoError);
        QString strReply = (QString) reply->readAll();
        qDebug() << strReply;
    });
    connect(reply, &QNetworkReply::finished, reply, &QNetworkReply::deleteLater);
...
{
    auto reply = m_auth->getGoogle()->deleteResource(this->getUrlEvent(m_calendarId, eventId));
    //ukoliko je uspesan delete vraca se prazan objekat
    connect(reply,&QNetworkReply::finished, [=](){
        qDebug() << "DELETE REQUEST FINISHED. Error? " << (reply->error() != QNetworkReply::NoError);
        QString strReply = (QString) reply->readAll();
        qDebug() << strReply;
    });
    connect(reply, &QNetworkReply::finished, reply, &QNetworkReply::deleteLater);
...
void CalendarManager::getCalendarIds() {
    ...
    auto reply = m_auth->getGoogle()->get(QUrl("https://www.googleapis.com/calendar/v3/users/me/calendarList/"));
    connect(reply, &QNetworkReply::finished, [=](){

        qDebug() << "GET CALENDAR IDS REQUEST FINISHED. Error? " << (reply->error() != QNetworkReply::NoError);
        QString strReply = (QString) reply->readAll();
...
void CalendarManager::getEventsFromCalendar(QString calendarId) {
    ...
    connect(reply, &QNetworkReply::finished, [=](){
        ...
        qDebug() << "GET EVENTS REQUEST FINISHED. Error? " << (reply->error() != QNetworkReply::NoError);
        QString strReply = (QString) reply->readAll();
        QJsonDocument jsonDocument = QJsonDocument::fromJson(strReply.toUtf8());
        m_jsonObject = new QJsonObject(jsonDocument.object());
...
void CalendarManager::grantAccess()
{
    connect(m_auth->getGoogle(), &QOAuth2AuthorizationCodeFlow::granted, [=](){
        ...
        QString fileName = QDir::currentPath() + "/../calendar/resources/tokenCalendar.json";
        QFile file(fileName);
...
QVector<QString> CalendarManager::getIds() const
{
    return m_ids;
}
...
const QUrl CalendarManager::getUrlEvents(QString calendarId) {
    return QUrl(QString("https://www.googleapis.com/calendar/v3/calendars/%1/events").arg(calendarId));
}

const QUrl CalendarManager::getUrlEvent(QString calendarId, QString eventId) {
    return QUrl(QString("https://www.googleapis.com/calendar/v3/calendars/%1/events/%2").arg(calendarId).arg(eventId));
}

QString CalendarManager::getCalendarId() const
{
    return m_calendarId;
}
