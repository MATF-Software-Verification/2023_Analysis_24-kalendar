...
AttachmentManager::AttachmentManager() {
  ...
  connect(m_auth->getGoogle(), &QOAuth2AuthorizationCodeFlow::tokenChanged,
          this, &AttachmentManager::onTokenChanged);
  m_auth->refreshAccessToken();
}

void AttachmentManager::grantAccess() {
  connect(m_auth->getGoogle(), &QOAuth2AuthorizationCodeFlow::granted, this,
          &AttachmentManager::onGranted);
...
void AttachmentManager::uploadAttachment(QFile *file) {
  ...
  metaDataPart.setHeader(
      ...
      QVariant(QString("form-data; name=\"%1\"").arg("metaData")));
  ...
  filePart.setHeader(
      QNetworkRequest::ContentDispositionHeader,
      QVariant(QString("form-data; name=\"%1\"").arg("fileContent")));
  ...
  QNetworkRequest request(QUrl("https://www.googleapis.com/upload/drive/v3/"
                               "files?uploadType=multipart&fields=webViewLink,"
                               "mimeType,name,id,iconLink"));
  request.setRawHeader("Authorization",
                       QString("Bearer %1").arg(token).toUtf8());
  request.setRawHeader(
      "Content-Type",
      (QString("multipart/related; charset=UTF-8; boundary=%1").arg(boundary))
          .toUtf8());
  ...
  connect(m_reply, &QNetworkReply::finished, this,
          &AttachmentManager::onFinished);
  connect(m_reply, &QNetworkReply::finished, m_reply,
...
void AttachmentManager::onGranted() {
  ...
  QString fileName =
      QDir::currentPath() + "/../calendar/resources/tokenDrive.json";
  QFile file(fileName);
...
void AttachmentManager::onFinished() {
  qDebug() << "GOOGLE DRIVE POST REQUEST FINISHED. Error? "
  ...
  attachmnets.push_back(new AttachmentInfo(
      jsonObject["id"].toString(), jsonObject["webViewLink"].toString(),
      jsonObject["mimeType"].toString(), jsonObject["name"].toString(),
      jsonObject["iconLink"].toString()));
  emit fileUploaded();
}
...
OAuth2 *AttachmentManager::auth() const { return m_auth; }

void AttachmentManager::setAuth(OAuth2 *auth) { m_auth = auth; }
