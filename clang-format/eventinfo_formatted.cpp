...
EventInfo::EventInfo(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::EventInfo) {
  ui->setupUi(this);
  setAttribute(Qt::WA_StyledBackground, true);
}

EventInfo::~EventInfo() { delete ui; }
...
void EventInfo::setEventData(QString id) {
  ...
  if (event.get_startDate() == event.get_endDate()) {
    dateNtime = QLocale(QLocale::English, QLocale::Serbia)
                    .toString(event.get_startDate());
    dateNtime = dateNtime + " : " + event.get_startTime().toString() + " - " +
                event.get_endTime().toString();
  } else {
    dateNtime = QLocale(QLocale::English, QLocale::Serbia)
                    .toString(event.get_startDate());
    dateNtime = dateNtime + ", " + event.get_startTime().toString() + " -\n";
    dateNtime = dateNtime +
                QLocale(QLocale::English, QLocale::Serbia)
                    .toString(event.get_endDate()) +
                ", " + event.get_endTime().toString();
  }
  ...
  if (event.m_repeat == Event::Dont_repeat)
    ...
    if (event.m_repeat == Event::Days)
      ...
      repeatString = "years";
    QString str = QString("Repeats every %1 %2, %3 times")
                      .arg(QString::number(event.getInterval()))
                      .arg(repeatString)
                      .arg(QString::number(event.getDuration()));
  ...
  if (event.getNoAlert()) {
    ...
    if (event.getAlertNum() == 0)
      ...
      alertString = "Alert is set " +
                    QString::number(event.getAlertNum() / 10080) +
                    " weeks before event";
    } else if (event.getAlertNum() % 1440 == 0) {
      alertString = "Alert is set " +
                    QString::number(event.getAlertNum() / 1440) +
                    " days before event";
    } else if (event.getAlertNum() % 60 == 0) {
      alertString = "Alert is set " +
                    QString::number(event.getAlertNum() / 60) +
                    " hours before event";
    } else if (event.getAlertNum() < 60) {
      alertString = "Alert is set " + QString::number(event.getAlertNum()) +
...
void EventInfo::on_pbUpdate_clicked() {
  ...
  ew->setEvent(
      event.get_title(), event.get_startDate(), event.get_endDate(),
      event.get_startTime(), event.get_endTime(), event.get_description(),
      event.getAlertType(), event.getAlertNum(), event.recurrenceToString(),
      event.getDuration(), event.getInterval(), event.getAttachments());
