...
CalendarWindow::~CalendarWindow() { delete ui; }
...
void CalendarWindow::on_calendarWidget_clicked(const QDate &date) {
  ...
  foreach (class Event *e, *myActivitiesList.getWeekEvents(date)) {
    ...
    ui->tableWidget->setSpan(startTime, dayOfTheWeekS, rowSpan,
                             dayOfTheWeekE - dayOfTheWeekS + 1);
  }
}
...
void CalendarWindow::setCalendarWidgetProperties() {
  ...
  QTextCharFormat formatSat =
      ui->calendarWidget->weekdayTextFormat(Qt::Saturday);
